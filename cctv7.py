    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        label = f'{id}'
                        if names[c] == 'bus' or names[c] == 'truck' or names[c] == 'car':
                            count_speed(bboxes, w, h, id, im0)
                        if names[c] == 'car':
                        ...
                        track_speed(bboxes, w, h, id)
                        if int(output[1]+(output[3]-output[1])/2) > (200) and int(output[1]+(output[3]-output[1])/2) < (300) and int(output[0]+(output[2]-output[0])/2) < (750) and int(output[0]+(output[2]-output[0])/2) > (60):
                            annotator.box_label(bboxes, label, color=colors(c, True))
            ...
            if show_vid:
                ...
                cv2.line(im0, start_point4, end_point4, color1, thickness=2)
                cv2.line(im0, start_point5, end_point5, color1, thickness=2)
                thickness = 2
                org = (20, 30)
                org1 = (20, 60)
                org2 = (20, 90)
                ...
                while t6 == t7:
                    ...
                    # sql = "INSERT INTO off_ramp_tallo_timur(date, Mobil, Bus_Truk, total) VALUES (now(), '" + \
                        # car+"', '"+truck+"', '"+count+"')"
                    sql = "INSERT INTO table_counting(date, Mobil, Bus_Truk, total, lokasi) VALUES (now(), '" + \
                        car1+"', '"+truck1+"', '"+count1+"', '"+lokasi+"')"
...
def count_obj1(box, w, h, id):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (125) and int(box[1]+(box[3]-box[1])/2) < (200):
        if id not in data:
            ...
            #     car1 = 0
            #     truck1 = 0
def track_speed(box, w, h, id):
    global count1, data, t7, t6, truck1, car1
    center_coordinates = (
        int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if int(box[1]+(box[3]-box[1])/2) > (200) and int(box[1]+(box[3]-box[1])/2) < (300) and (60) > int(box[0]+(box[2]-box[0])/2)  and (750) < int(box[0]+(box[2]-box[0])/2):
        if id not in data:
            count1 = count1
            data.append(id)
...
def count_speed(box, w, h, id, im0): #speed down
    global count1, data, t7, t6, truck1, car1, person
    center_coordinates = (int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if 200 < int(center_coordinates[1]) + offset and int(center_coordinates[1] ) < 200 + offset and 65 < int(center_coordinates[0])  and int(center_coordinates[0]) < 750:
        vh_t[id] = time.time()
        if id in vh_t:
            if 300 < int(center_coordinates[1]) + offset and int(center_coordinates[1] ) < 300 + offset and 65 < int(center_coordinates[0])  and int(center_coordinates[0]) < 750:
                elapsed_time = time.time() - vh_t[id]
                cv2.line(im0, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (255, 0, 0), thickness=2)
                distance = 2  # meter 
                speed_mps = distance / elapsed_time
                speed_kmh = speed_mps * 3.6
                text = str(int(speed_kmh))
                cv2.putText(im0, text + 'Km/h', (int(center_coordinates[0])+50, int(center_coordinates[1])-50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
...
# def count_speed (distance, time, box, w, h, id, im0):
#     global count1, data, t7, t6, truck1, car1, person
#     center_coordinates = (int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
#     if 200 > int(box[1]+(box[3]-box[1])/2) and 300 < int(box[1]+(box[3]-box[1])/2):
#         vh_t[id] = time.time()
#         if 60 > int(box[0]+(box[2]-box[0])/2)  and 750 < int(box[0]+(box[2]-box[0])/2):
#             elapsed_time = time.time() - vh_t
#             cv2.line(im0, (int(box[0]),int(box[1])),(int(box[2]),int(box[3])) , (255,0,0), thickness=2)
#             distance = 10 #meter
#             speed_mps = distance / elapsed_time
#             speed_kmh = speed_mps * 3.6
#             text = str(int(speed_kmh))
#             cv2.putText(im0, text + 'Km/h', (300,80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
